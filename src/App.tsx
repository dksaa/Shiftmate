...
interface DailyShifts {
  ...
  Middle: Employee[];
  Malam: Employee[];
  [key: string]: Employee[]; // Index signature untuk akses dinamis
}
...
const App = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-800 font-sans pb-20 overflow-x-hidden">
      ...
      {showUrlInput && <div className="bg-white p-4 border-b animate-in slide-in-from-top-2"><div className="max-w-5xl mx-auto"><input className="border w-full p-2 text-xs rounded outline-none" value={scriptUrl} onChange={e=>setScriptUrl(e.target.value)} placeholder="URL Google Script..." /></div></div>}
          ...
          <div className="space-y-6">
            <div className="bg-white p-5 rounded-xl border shadow-sm">
               ...
               <div className="flex flex-col sm:flex-row gap-3 mb-4">
                 <button onClick={()=>setDuration('1day')} className={`flex-1 py-3 border rounded-lg font-bold text-xs transition-colors ${duration==='1day' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-200 hover:bg-slate-50'}`}>1 Hari</button>
                 <button onClick={()=>setDuration('1week')} className={`flex-1 py-3 border rounded-lg font-bold text-xs transition-colors ${duration==='1week' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-200 hover:bg-slate-50'}`}>1 Minggu (Rotasi)</button>
               </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-900 text-white p-5 rounded-xl shadow-xl gap-4">
               ...
               <div className="flex w-full sm:w-auto gap-2"><button onClick={()=>setStep(2)} className="flex-1 sm:flex-none bg-white/10 px-4 py-2 rounded-lg text-xs font-bold hover:bg-white/20 transition-colors">Edit Config</button><button onClick={saveToGoogleSheets} disabled={saveStatus==='loading'||saveStatus==='success'} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold shadow flex gap-2 items-center justify-center transition-colors ${saveStatus==='success'?'bg-emerald-500 text-white':'bg-indigo-600 hover:bg-indigo-500 text-white'}`}><Icons.Save/> {saveStatus==='loading'?'Menyimpan...':saveStatus==='success'?'Tersimpan':'Simpan ke Sheets'}</button></div>
             </div>

             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
               {schedule.map((day, dIdx) => (
                 <div key={dIdx} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col relative hover:shadow-md transition-shadow">
